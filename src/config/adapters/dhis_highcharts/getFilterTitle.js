        ...
        filters.forEach(filter => {
            ...
            if (isOuFilterWithLevels || isOuFilterWithGroups) {
                ...
                if (isArray(filterItems)) {
                    const hasMetadata = filterItems.some(id => Boolean(metaData.items[id]));

                    if (hasMetadata) {
                        const sectionParts = filterItems.map(id => metaData.items[id].name);
                        ...
                        titleParts.push(metaData.items[filter.dimension].name + ': ' + filterItems.join(', '));
                    }
                }
            }
        });
